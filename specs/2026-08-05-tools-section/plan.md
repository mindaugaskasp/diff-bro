...
| ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| QA     | **Rotation's decrypt invariant was asserted by a counter.** Replacing `decryptionIdentities()` with `[await getIdentity()]` kept `retiredKeyCount() === 1` and both rotation specs green while orphaning every diff sealed to the pre-rotation key. | `key-rotation.spec.mjs` gained a two-profile test: Alice seals for Bob, Bob rotates, Bob opens it. **Proven** by making that exact edit — the two original tests still passed, the new one failed with `not-for-you`.                                                                                            |
| Eng    | **`useCopyFeedback` bypassed by ten tool panels**, each with a 900ms timer against the composable's 1200, none clearing a previous timer or disposing on unmount.                                                                                   | All ten converted. `flash(mark)` added so the four panels that record WHICH button was pressed keep that behaviour instead of a second pattern; two tests cover the mark and the restart.                                                                                                                        |
| Sec    | **`structuralDiff` recursion had no depth cap**, and `xmlValue` built a plain object from attacker-controlled element names — one named `__proto__` re-pointed the prototype and the element vanished from the comparison.                          | `MAX_TREE_DEPTH` bounds `canonical`/`walk`/`compare`; `Object.create(null)` for the XML map. Both tested, the `__proto__` one **proven** red→green. `visibleStructureRows` moved to `structureRows.js` to pay for the lines rather than raise the cap — the file went 300 → 279 and the ratchet was retightened. |
| Sec    | `sweepGitTemp` used `statSync`, so a planted symlink was aged by its target; git temp files were swept on launch only.                                                                                                                              | `lstatSync` + non-directories skipped, and a `will-quit` sweep alongside the launch one.                                                                                                                                                                                                                         |
| Sec    | `git:register` / `cli:install` wrote an executable onto PATH and rewrote `~/.gitconfig` with no confirmation, unlike every `openExternal` site.                                                                                                     | Both confirm in main first. `stubMessageBox` added to the e2e fixtures so specs can answer.                                                                                                                                                                                                                      |
| QA     | `openSnippets('not an object')` asserted `.resolves.toBeTruthy()` — satisfied by `{ ok: true }` on hostile input.                                                                                                                                   | Asserts the exact verdict.                                                                                                                                                                                                                                                                                       |
| QA     | `mermaid.test.js` computed its expectation with the expression under test, so it could not fail for any theme.                                                                                                                                      | Replaced with a literal 14-row table plus a check that the table covers `THEMES`.                                                                                                                                                                                                                                |
| QA     | `emailStore.test.js` never reset the module-level `pending`, leaking a plaintext draft into every later test in the file.                                                                                                                           | `cancel()` in `beforeEach`.                                                                                                                                                                                                                                                                                      |
| QA     | `shareStore.shareTo`, `structureAdapter` and `uiStore`'s three actions had **no unit test at all** (54.8% / 25% / 33%).                                                                                                                             | Three new files, 15 tests — including the `localCopy === false` branch that decides whether the sender is told they have no copy of what they sent, and the palette-scope pairing nothing asserted.                                                                                                              |
| QA     | Coverage floors stale (93/86/92/95 against 94.9/87.7/95.8/95.9).                                                                                                                                                                                    | Raised to **94/87/95/95**. Widening the set to the nine tested `src/main/` modules is NOT done: it drops the aggregate to 93.6/86.6/94.4/94.5, and buying that would mean lowering the `lines` floor. They need tests first — recorded in `vitest.config.mjs`.                                                   |
| Eng    | ~130 lines of dead favourites-shelf CSS, a dead fourth `.btn-icon` (`.icon-btn`), and four unused `ui.css` classes.                                                                                                                                 | Deleted. The three sidebar stylesheets went 184/71/40 → 29/31/26 lines.                                                                                                                                                                                                                                          |
| Eng    | The same `JSON.parse(loadPersisted(...))` + catch in five stores.                                                                                                                                                                                   | `loadPersistedJson` in `persist.js`, adopted where the shape matched.                                                                                                                                                                                                                                            |
| Sec    | `docs/standards.md` rule 7 named three path surfaces; a fourth (`appData.js` `datadir:reveal`) had arrived.                                                                                                                                         | Rule 7 corrected.                                                                                                                                                                                                                                                                                                |

Two conventions were also written down as standing rules at the user's request:
**"Do not reinvent what a package already does"** (with the network audit as the
only veto, and the requirement to record what was rejected and why) and
**"Naming a function by what it gives back"** (a returning function says so, a
boolean reads as a question, a mutator is an imperative that does not double as a
getter — `get` is explicitly NOT a blanket prefix). Five predicates named as
statements were renamed under the second: `finite` → `isFiniteNumber`, `sameAt` →
`isSameAt`, `same` → `isSameRect`, `skipped` → `isSkipped`,
`defaultSemanticView` → `shouldOpenSemantic`. A date library was **evaluated and
rejected** under the first, with the reason recorded at `relativeTime.js`.
