...
| **The merge write takes TEXT, never a path**      | `merge:write` writes over a file the user already had, and can only write the row the view has open — the `$MERGED` path main was launched with, or a row of the conflicts list main built. The renderer sends the resolved text; there is no argument for a filename. With no merge launch in progress the handler writes nothing at all, and one launch permits one write                                                  | `mergeSession.js`, `mergeRoute.js`                           |
| **A conflicted file is named by INDEX**           | `merge:take` and `merge:open` address a row by its position in the list main enumerated, never by path — a row carries only a display name and a directory label. Main recomputes the path from the repository root, and re-verifies it against `git diff --diff-filter=U` at WRITE time, so a row that stopped being unmerged since the dialog opened fails closed. `merge:list` returns rows; `merge:endWalk` forgets them | `conflictSession.js`, `conflictList.js`, `mergeRoute.js`     |
