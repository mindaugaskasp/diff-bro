...
const status = computed(() => (input.value.trim() ? validateXml(input.value) : null))
const statusText = computed(() => {
  if (!status.value) return ''
  if (status.value.valid) return 'Valid XML'
...
function format() {
  if (status.value?.valid) input.value = formatXml(input.value)
}
